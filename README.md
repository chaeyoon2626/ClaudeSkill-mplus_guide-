...
Download mplus-skill.skill from this repository (or download the whole repo as a ZIP via "Code → Download ZIP" — either works).
In the Claude app, go to Settings → Capabilities and turn on "Code execution" if it's off. (On a Team/Enterprise account, an admin may need to enable Skills in organization settings first.)
Go to Customize → Skills, click "+" → "Create skill" → "Upload a skill".
Upload mplus-skill.skill as-is. (If the upload dialog rejects the .skill extension, just rename the file to mplus-skill.zip — the contents are identical.)
Toggle the skill on after upload, and it's ready to use.
2) Claude Code
Download mplus-skill.skill (or the whole repo) and unzip it. Confirm that SKILL.md sits directly inside the mplus-skill/ folder.

Copy that folder to one of these locations:
...
Restart Claude Code or start a new session and it will pick it up automatically. Invoke it directly with /mplus-manual-guide, or just ask something like "I want to run a moderated mediation analysis in Mplus" and it triggers automatically.
Usage examples
...
Confirms variable types (continuous/categorical) and how missing data is coded
Confirms the path diagram: stress (rectangle) → sleep (rectangle) → depress (rectangle), with support moderating the stress → sleep path (interaction term)
Only after you confirm "yes, that's right" does it give the minimal code:
...
Example follow-up: "I also want a bootstrap confidence interval" → adds only ANALYSIS: BOOTSTRAP = 1000; and OUTPUT: CINTERVAL (BOOTSTRAP); on top of the existing code (nothing else gets added on its own).
...
Confirms the 6 items are categorical/ordinal, and how missing data is coded
Confirms the path diagram: a circle (dep) pointing to 6 rectangles (d1–d6), with a residual arrow on each item
Once confirmed, minimal code:
...
Example follow-ups: "I want to see fit indices" → adds OUTPUT: STDYX; / "I also want modification indices" → adds OUTPUT: MODINDICES;
...
Confirms the clustering variable (school ID), how many indicator variables define the profiles, and how missing data is coded
Confirms the path diagram: a dashed line separating Within/Between, a circle (c, the latent class variable) pointing to the indicator rectangles at the Within level, and the school clustering shown at the Between level
Once confirmed, minimal code:
...
Example follow-up: "How do I check how many classes/profiles is right?" → explains and adds TECH11/TECH14 (LMR and bootstrap likelihood ratio tests) with the manual as the basis.
Limitations
...
License
This repository (README, SKILL.md, references/*.md, and other original content) is distributed under the MIT License. Mplus itself and the official User's Guide are copyrighted by Muthén & Muthén.
